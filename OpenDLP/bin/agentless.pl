...
my $string = "SELECT username,password,domain,exts,ignore_exts,dirs,ignore_dirs,regex,debug,creditcards,zipfiles,memory,mask,hash,scantype,email_addr,gpgpassword from profiles where profile=?";
my $sth_local = $dbh_local->prepare( $string );
...
my $results = $sth_local->fetchrow_hashref();
#if( $$results[14] ne "" )
if($sth_local->rows == 1)
{
    #########################
    #NEW: Josh 2018.07.12
    #########################
    my $emailAddresses = $results->{"email_addr"};
    
	$username = $results->{"username"};
	#$password = $results->{"password"};
	$domain = $results->{"domain"};
	$exts = $results->{"exts"};
	$ignore_exts = $results->{"ignore_exts"};
	$dirs = $results->{"dirs"};
	$ignore_dirs = $results->{"ignore_dirs"};
	$regex = $results->{"regex"};
	$debug = $results->{"debug"};
	$creditcards = $results->{"creditcards"};
	$zipfiles = $results->{"zipfiles"};
	...
	{
        send_email($emailAddresses, $scanname);
	}
}
else
{
...
sub send_email
{
    ...
    if($doSend eq 1)
    {
        #my $string = "select sys.scan, sys.scantype, sys.profile, sys.ip, sys.dbtotal, sys.tabletotal, sys.scantype, count(r.tracker) as matches from systems sys join results r on sys.tracker = r.tracker and sys.scan = ?";
        my $string = "select sys.scan, sys.scantype from systems sys join results r on sys.tracker = r.tracker and sys.scan = ?";
        my $message = "";
        my $scanName = "";
        my $scanType = "";
        ...
        #$message .= "<tr><th>Profile</th><th>IP</th><th>Databases Scanned</th><th>Tables Scanned</th><th>Matches</th></tr>";
        while(my $results = $sth->fetchrow_hashref())
        {
            $scanName = $results->{"scan"};
            $scanType = $results->{"scantype"};
        #    $message .= "<tr><td>" . $results->{"profile"}   . "</td>";
        #    $message .= "<td>" . $results->{"ip"}           . "</td>";
        #    $message .= "<td>" . $results->{"dbtotal"}   . "</td>";
        #    $message .= "<td>" . $results->{"tabletotal"}    . "</td>";
        #    $message .= "<td>" . $results->{"matches"}      . "</td></tr>";
        }
        #$message .= "</table>";
        
        open(my $MAIL, "|/usr/sbin/ssmtp -t");
        print $MAIL "To: $dst\n";
        print $MAIL "From: $src\n";
        print $MAIL "MIME-Version:1.0\n";
        print $MAIL "Content-Type: text/html; charset=utf-8\n";
        print $MAIL "Subject: OpenDLP Scan Results: $scanName\n\n";
        #print $MAIL "<h1>Results for Scan \"$scanName\" ($scanType)</h1>\n$message\n";
        print $MAIL "<h1>Results for Scan \"$scanName\" ($scanType)</h1>\nTODO\n";
        close($MAIL);
    }
    $sth->finish();
