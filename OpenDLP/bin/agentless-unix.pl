...
my $string = "SELECT username,password,domain,exts,ignore_exts,dirs,ignore_dirs,regex,debug,creditcards,zipfiles,memory,mask,scantype,email_addr,gpgpassword from profiles where profile=?";
my $sth_local = $dbh_local->prepare( $string );
