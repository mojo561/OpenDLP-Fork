...
my $string = "select username,password,mask,ignore_dbs,dbs,ignore_tables,tables,ignore_columns,columns,rows,scantype,regex,creditcards,debug,email_addr,gpgpassword FROM profiles where profile=?";
...
if($sth_local->rows == 1)
{
	...
	$mask          = $results->{"mask"};
	$ignore_dbs    = $results->{"ignore_dbs"};
	$dbs           = $results->{"dbs"};
	$ignore_tables = $results->{"ignore_tables"};
	$tables        = $results->{"tables"};
	$ignore_columns= $results->{"ignore_columns"};
